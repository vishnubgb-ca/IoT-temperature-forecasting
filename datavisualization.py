...
def data_visualization():
    ...
    for col in columns:
        ...
        fig.show()
        # fig.write_image(f"series_{col}_8.jpg")
    start=DT.datetime(2016, 9, 12,0,0)
    end=DT.datetime(2016, 9, 12,9,30)
    for col in columns:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data[start:end].index, y=data[start:end][col], mode='lines', name=col))
        fig.update_layout(title=f"Plot of {col} on 12th September",template='plotly_dark')
        fig.update_xaxes(showgrid=False,zeroline=False)
        fig.update_yaxes(showgrid=False,zeroline=False)
        fig.show()
